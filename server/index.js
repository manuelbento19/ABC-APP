...
app.post('/register', (req, res) => {
  ...
  if (!['empresa', 'empreendedor'].includes(tipo)) {
    res.status(400).json({ error: 'Tipo de usuário inválido. Use "empresa" ou "empreendedor".' });
    return;
  }
  ...
  db.query('INSERT INTO Usuario (Email, Endereco, Telefone, AreaAtuacao, Provincia, Senha, tipo) VALUES (?, ?, ?, ?, ?, ?, ?)', 
    ...
    (err, results) => {
      ...
      if (tipo === 'empresa') {
        ...
        db.query('INSERT INTO Empresa (NomeRepresentante, NomeEmpresa, NIF, AnosDeExistencia, UsuarioID) VALUES (?, ?, ?, ?, ?)', 
          ...
          (err, results) => {
            ...
            res.status(201).json({ message: 'Usuário e Empresa criados com sucesso', id: usuarioId });
          }
        );
      } else if (tipo === 'empreendedor') {
        const { Nome, Genero, DataNascimento, BI } = req.body;
        db.query('INSERT INTO Empreendedor (Nome, Genero, DataNascimento, BI, UsuarioID) VALUES (?, ?, ?, ?, ?)', 
          ...
          (err, results) => {
            ...
            res.status(201).json({ message: 'Usuário e Empreendedor criados com sucesso', id: usuarioId });
          }
        );
      }
    }
...
app.put('/empresas/:id', (req, res) => {
  const id = req.params.id;
  const { NomeRepresentante, NomeEmpresa, NIF, AnosDeExistencia, UsuarioID } = req.body;
  db.query('UPDATE Empresa SET NomeRepresentante = ?, NomeEmpresa = ?, NIF = ?, AnosDeExistencia = ?, UsuarioID = ? WHERE id = ?', [NomeRepresentante, NomeEmpresa, NIF, AnosDeExistencia, UsuarioID, id], (err, results) => {
    if (err) {
      console.error('Erro ao atualizar a empresa:', err);
...
app.put('/empreendedores/:id', (req, res) => {
  ...
  db.query('UPDATE Empreendedor SET Nome = ?, Genero = ?, DataNascimento = ?, BI = ?, UsuarioID = ? WHERE id = ?', [Nome, Genero, DataNascimento, BI, UsuarioID, id], (err, results) => {
    if (err) {
      console.error('Erro ao atualizar o empreendedor:', err);
...
app.post('/servicos', (req, res) => {
  ...
  db.query('INSERT INTO Servicos (Descricao, Preco, CategoriaID, UsuarioID, Foto) VALUES (?, ?, ?, ?, ?)', [Descricao, Preco, CategoriaID, UsuarioID, Foto], (err, results) => {
    if (err) {
      console.error('Erro ao criar o serviço:', err);
      res.status(500).json({ error: 'Erro ao criar o serviço' });
      return;
    }
    res.status(201).json({ message: 'Serviço criado com sucesso', id: results.insertId });
  });
});
...
app.put('/servicos/:id', (req, res) => {
  const id = req.params.id;
  const { Descricao, Preco, CategoriaID, UsuarioID, Foto } = req.body;
  db.query('UPDATE Servicos SET Descricao = ?, Preco = ?, CategoriaID = ?, UsuarioID = ?, Foto = ? WHERE id = ?', [Descricao, Preco, CategoriaID, UsuarioID, Foto, id], (err, results) => {
    if (err) {
...
app.get('/categorias', (req, res) => {
  db.query('SELECT * FROM CategoriaServico', (err, results) => {
    if (err) {
      ...
      res.status(500).json({ error: 'Erro ao obter as categorias de serviços' });
      return;
    }
    res.json(results);
  });
...
app.get('/categorias/:id', (req, res) => {
  const id = req.params.id;
  db.query('SELECT * FROM CategoriaServico WHERE id = ?', [id], (err, results) => {
    if (err) {
      console.error('Erro ao obter a categoria de serviços:', err);
      res.status(500).json({ error: 'Erro ao obter a categoria de serviços' });
      return;
    }
    if (results.length === 0) {
...
app.post('/categorias', (req, res) => {
  ...
  db.query('INSERT INTO CategoriaServico (NomeCategoria, DescricaoCategoria) VALUES (?, ?)', [NomeCategoria, DescricaoCategoria], (err, results) => {
    if (err) {
      console.error('Erro ao criar a categoria de serviços:', err);
      res.status(500).json({ error: 'Erro ao criar a categoria de serviços' });
      return;
    }
    res.status(201).json({ message: 'Categoria de serviços criada com sucesso', id: results.insertId });
  });
});
...
app.put('/categorias/:id', (req, res) => {
  const id = req.params.id;
  const { NomeCategoria, DescricaoCategoria } = req.body;
  db.query('UPDATE CategoriaServico SET NomeCategoria = ?, DescricaoCategoria = ? WHERE id = ?', [NomeCategoria, DescricaoCategoria, id], (err, results) => {
    if (err) {
      console.error('Erro ao atualizar a categoria de serviços:', err);
      res.status(500).json({ error: 'Erro ao atualizar a categoria de serviços' });
      return;
    }
    if (results.affectedRows === 0) {
      res.status(404).json({ error: 'Categoria de serviços não encontrada' });
...
app.delete('/categorias/:id', (req, res) => {
  const id = req.params.id;
  db.query('DELETE FROM CategoriaServico WHERE id = ?', [id], (err, results) => {
    if (err) {
      console.error('Erro ao excluir a categoria de serviços:', err);
      res.status(500).json({ error: 'Erro ao excluir a categoria de serviços' });
      return;
    }
    if (results.affectedRows === 0) {
      res.status(404).json({ error: 'Categoria de serviços não encontrada' });
...
app.post('/avaliacoes', (req, res) => {
  ...
  db.query('INSERT INTO Avaliacao (Nota, Comentario, UsuarioID, ServicoID) VALUES (?, ?, ?, ?)', [Nota, Comentario, UsuarioID, ServicoID], (err, results) => {
    if (err) {
      console.error('Erro ao criar a avaliação:', err);
      res.status(500).json({ error: 'Erro ao criar a avaliação' });
      return;
    }
    res.status(201).json({ message: 'Avaliação criada com sucesso', id: results.insertId });
  });
});
...
app.put('/avaliacoes/:id', (req, res) => {
  const id = req.params.id;
  const { Nota, Comentario, UsuarioID, ServicoID } = req.body;
  db.query('UPDATE Avaliacao SET Nota = ?, Comentario = ?, UsuarioID = ?, ServicoID = ? WHERE id = ?', [Nota, Comentario, UsuarioID, ServicoID, id], (err, results) => {
    if (err) {
...
app.post('/feedback', (req, res) => {
  ...
  db.query('INSERT INTO Feedback (Comentario, Data, AvaliacaoID) VALUES (?, ?, ?)', [Comentario, Data, AvaliacaoID], (err, results) => {
    if (err) {
      console.error('Erro ao criar o feedback:', err);
      res.status(500).json({ error: 'Erro ao criar o feedback' });
      return;
    }
    res.status(201).json({ message: 'Feedback criado com sucesso', id: results.insertId });
  });
});
...
app.put('/feedback/:id', (req, res) => {
  const id = req.params.id;
  const { Comentario, Data, AvaliacaoID } = req.body;
  db.query('UPDATE Feedback SET Comentario = ?, Data = ?, AvaliacaoID = ? WHERE id = ?', [Comentario, Data, AvaliacaoID, id], (err, results) => {
    if (err) {
...
app.get('/salas/:id', (req, res) => {
  const id = req.params.id;
  db.query('SELECT * FROM SalaDeNegocios WHERE id = ?', [id], (err, results) => {
    if (err) {
...
app.post('/salas', (req, res) => {
  ...
  db.query('INSERT INTO SalaDeNegocios (Titulo, Descricao, Link, Anexos, UsuarioID) VALUES (?, ?, ?, ?, ?)', [Titulo, Descricao, Link, Anexos, UsuarioID], (err, results) => {
    if (err) {
      console.error('Erro ao criar a sala de negócios:', err);
      res.status(500).json({ error: 'Erro ao criar a sala de negócios' });
      return;
    }
    res.status(201).json({ message: 'Sala de negócios criada com sucesso', id: results.insertId });
  });
});
...
app.put('/salas/:id', (req, res) => {
  const id = req.params.id;
  const { Titulo, Descricao, Link, Anexos, UsuarioID } = req.body;
  db.query('UPDATE SalaDeNegocios SET Titulo = ?, Descricao = ?, Link = ?, Anexos = ?, UsuarioID = ? WHERE id = ?', [Titulo, Descricao, Link, Anexos, UsuarioID, id], (err, results) => {
    if (err) {
...
app.post('/participacoes', (req, res) => {
  ...
  db.query('INSERT INTO Participacao (UsuarioID, SalaDeNegociosID, DataParticipacao) VALUES (?, ?, ?)', [UsuarioID, SalaDeNegociosID, DataParticipacao], (err, results) => {
    if (err) {
      console.error('Erro ao criar a participação:', err);
      res.status(500).json({ error: 'Erro ao criar a participação' });
      return;
    }
    res.status(201).json({ message: 'Participação criada com sucesso', id: results.insertId });
  });
});
...
app.put('/participacoes/:id', (req, res) => {
  const id = req.params.id;
  const { UsuarioID, SalaDeNegociosID, DataParticipacao } = req.body;
  db.query('UPDATE Participacao SET UsuarioID = ?, SalaDeNegociosID = ?, DataParticipacao = ? WHERE id = ?', [UsuarioID, SalaDeNegociosID, DataParticipacao, id], (err, results) => {
    if (err) {
...
app.post('/conexoes', (req, res) => {
  ...
  db.query('INSERT INTO Conexao (Usuario1ID, Usuario2ID, DataConexao) VALUES (?, ?, ?)', [Usuario1ID, Usuario2ID, DataConexao], (err, results) => {
    if (err) {
      console.error('Erro ao criar a conexão:', err);
      res.status(500).json({ error: 'Erro ao criar a conexão' });
      return;
    }
    res.status(201).json({ message: 'Conexão criada com sucesso', id: results.insertId });
  });
});
...
app.put('/conexoes/:id', (req, res) => {
  const id = req.params.id;
  const { Usuario1ID, Usuario2ID, DataConexao } = req.body;
  db.query('UPDATE Conexao SET Usuario1ID = ?, Usuario2ID = ?, DataConexao = ? WHERE id = ?', [Usuario1ID, Usuario2ID, DataConexao, id], (err, results) => {
    if (err) {
...
app.post('/mensagens', (req, res) => {
  const { RemetenteID, DestinatarioID, Conteudo, DataEnvio, Lida, Anexos } = req.body;
  db.query('INSERT INTO Mensagem (RemetenteID, DestinatarioID, Conteudo, DataEnvio, Lida, Anexos) VALUES (?, ?, ?, ?, ?, ?)', [RemetenteID, DestinatarioID, Conteudo, DataEnvio, Lida, Anexos], (err, results) => {
    if (err) {
      console.error('Erro ao criar a mensagem:', err);
      res.status(500).json({ error: 'Erro ao criar a mensagem' });
      return;
    }
    res.status(201).json({ message: 'Mensagem criada com sucesso', id: results.insertId });
  });
});
...
app.put('/mensagens/:id', (req, res) => {
  const id = req.params.id;
  const { RemetenteID, DestinatarioID, Conteudo, DataEnvio, Lida, Anexos } = req.body;
  db.query('UPDATE Mensagem SET RemetenteID = ?, DestinatarioID = ?, Conteudo = ?, DataEnvio = ?, Lida = ?, Anexos = ? WHERE id = ?', [RemetenteID, DestinatarioID, Conteudo, DataEnvio, Lida, Anexos, id], (err, results) => {
    if (err) {
...
app.post('/eventos', (req, res) => {
  const { Nome, Data, Local, UsuarioID } = req.body;
  db.query('INSERT INTO Eventos (Nome, Data, Local, UsuarioID) VALUES (?, ?, ?, ?)', [Nome, Data, Local, UsuarioID], (err, results) => {
    if (err) {
      console.error('Erro ao criar o evento:', err);
      res.status(500).json({ error: 'Erro ao criar o evento' });
      return;
    }
    res.status(201).json({ message: 'Evento criado com sucesso', id: results.insertId });
  });
});
...
app.put('/eventos/:id', (req, res) => {
  const id = req.params.id;
  const { Nome, Data, Local, UsuarioID } = req.body;
  db.query('UPDATE Eventos SET Nome = ?, Data = ?, Local = ?, UsuarioID = ? WHERE id = ?', [Nome, Data, Local, UsuarioID, id], (err, results) => {
    if (err) {
...
app.post('/administradores', (req, res) => {
  ...
  db.query('INSERT INTO Administrador (Nome, Genero, Email, Senha) VALUES (?, ?, ?, ?)', [Nome, Genero, Email, Senha], (err, results) => {
    if (err) {
...
app.put('/administradores/:id', (req, res) => {
  const id = req.params.id;
  const { Nome, Genero, Email, Senha } = req.body;
  db.query('UPDATE Administrador SET Nome = ?, Genero = ?, Email = ?, Senha = ? WHERE id = ?', [Nome, Genero, Email, Senha, id], (err, results) => {
    if (err) {
...
app.put('/postagens/:id', (req, res) => {
  ...
  db.query('UPDATE Postagem SET Conteudo = ?, Data = ?, UsuarioID = ?, Foto = ? WHERE id = ?', [Conteudo, Data, UsuarioID, Foto, id], (err, results) => {
    if (err) {
